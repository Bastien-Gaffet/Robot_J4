...
def train():
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # Tour de l'adversaire : meilleure version sauvegardée
            if not done:
                with torch.no_grad():
                    opp_state_tensor = torch.tensor(state, dtype=torch.float32, device=DEVICE).unsqueeze(0)
                    q_values_opp = best_dqn(opp_state_tensor)
                    for i in range(COLS):
                        if i not in env.valid_actions():
                            q_values_opp[0][i] = -float("inf")
                    opp_action = torch.argmax(q_values_opp).item()
                env.play(opp_action)
                done = env.game_over()
                state = env.get_state()
        ...
        if winrate > best_winrate:
            ...
            # Mise à jour du meilleur adversaire
            best_dqn.load_state_dict(dqn.state_dict())
