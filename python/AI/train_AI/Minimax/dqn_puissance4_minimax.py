...
def minimax(env, player, depth, maximizing):
    if depth == 0 or env.game_over():
        return compute_reward(env, player, -1), None  # -1 car pas de move à ce stade
    ...
    if maximizing:
        ...
        for move in valid_moves:
            ...
            eval_score, _ = minimax(new_env, player, depth - 1, True)
...
def train():
    ...
    # Meilleure version sauvegardée utilisée comme adversaire
    best_dqn = ConvDQN().to(DEVICE)
    if os.path.exists(MODEL_PATH):
        best_dqn.load_state_dict(torch.load(MODEL_PATH, weights_only=True))
    best_dqn.eval()
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # Tour de l'adversaire : Minimax profondeur 2
            if not done:
                _, opp_action = minimax(env, env.current_player, depth=2, maximizing=True)
        ...
        if winrate > best_winrate:
            ...
            export_to_onnx(dqn)
            best_dqn.load_state_dict(dqn.state_dict())  # Met à jour l'adversaire
...
def imminent_threat(env, player):
    # Retourne True si le joueur peut gagner au prochain coup
    for a in env.valid_actions():
        temp_env = Puissance4Env()
        temp_env.board = env.board.copy()
        temp_env.current_player = player
        temp_env.play(a)
        if temp_env.is_winning(player):
            return True
    return False
...
def compute_reward(env, player, action):
    ...
    reward = -0.01  # Base penalty to encourage faster wins

    # Bonus pour jouer au centre
    if action == COLS // 2:
        reward += 0.05

    # Récompense pour motifs favorables
    def count_patterns(length, target):
        count = 0
        for r in range(ROWS):
            for c in range(COLS - length + 1):
    ...
    reward += 0.005 * count_patterns(2, player)
    reward += 0.1 * count_patterns(3, player)
    reward -= 0.3 * count_patterns(3, opp)  # pénalise plus les menaces adverses

    # Détection des menaces imminentes de l'adversaire
    if imminent_threat(env, opp):
        reward -= 0.5  # forte pénalité si l'IA laisse une menace
...
def train():
    ...
    archive_dir = os.path.join("archives", f"session_{timestamp}")
    os.makedirs(archive_dir, exist_ok=True)
    archive_model_path = os.path.join(archive_dir, "dqn_puissance4.pth")
    archive_onnx_path = os.path.join(archive_dir, "puissance4_model.onnx")
    writer = SummaryWriter()
    env = Puissance4Env()
    dqn = ConvDQN().to(DEVICE)
    target_dqn = ConvDQN().to(DEVICE)
    ...
    for ep in range(episodes):
        ...
        if winrate > best_winrate:
            ...
            # Sauvegarde globale (overwritten)
            torch.save(dqn.state_dict(), "dqn_puissance4.pth")
            export_to_onnx(dqn)

            # Sauvegarde d'archive (unique)
            torch.save(dqn.state_dict(), archive_model_path)
            export_to_onnx(dqn)
            torch.onnx.export(dqn, torch.randn(1, 3, ROWS, COLS).to(DEVICE), archive_onnx_path,
